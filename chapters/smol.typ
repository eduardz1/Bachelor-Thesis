...
#_content(
  [
    ...
    In an FMO the in and out ports of the corresponding FMI become the fields of the object and the functions are converted in class methods, for example, the $mono("doStep")$ operation @fmi-chapter is translated to a method that accepts a floating point number as parameter (the time). For example to initialize an FMO `gr` in SMOL that accepts a Boolean as input and returns a Double as output, one would write:
    ...
    The number of inputs and outputs is not limited and their name has to match the name of the corresponding variable in the FMU. They have a `tick` method that accepts a floating point number as a parameter to advance the simulation by a given time.

    == Semantical Lifting
  
    Semantical Lifting maps a program state to a knowledge graph, to see the program state through the lens of domain knowledge @ksosu. At any point at runtime, the state of the program can be queried and lifted states can be used to check the consistency of the program with the domain constraints.
