...
ast:hasPlantId rdf:type owl:DatatypeProperty ;
    ...
    We can see that we define the namespaces at the top of the document to make the rest of the definition more readable, the namespaces enable us to use the standard definitions, for example when we use the `owl:ObjectProperty` class what we are using is the 
...
```
    ...
    The program is structured as a Python module. The main module is the `collector` module. It contains the `__main__.py` file which is the entry point of the program. It also contains the `config.py` file that is responsible for reading the configuration file and making it available to the rest of the program. The configuration file is a `.ini`, in the main module we include an example file that can be used as a template. The configuration file enables the user to configure the following parameters:
...
class Moisture:
    ...
    Having the function cached as a class variable enables us to reuse it for every value that needs to be converted, for a noticeable performance improvement.
